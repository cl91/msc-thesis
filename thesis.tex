...
We see from Fig.~\ref{stability1} that the option of having a breathing mode increases the area of stable oscillons. This is compared with numerical result in Fig.~\ref{stabilityfull}. The theoretical region of the $A$-$R$ diagram where stable oscillons exist is the region spanned by the inside of the stability curves with $d\in(-1,1)$. We can see from Fig.~\ref{stabilityfull} that the theoretical plot is consistent with the numerical plot.
...
\begin{figure}\centering
  ...
  \caption{The full stability map of the 1D $\phi^4$ oscillons. [TOP] Theoretical map with $d\in(-1,1)$. The dashed curve is the stability curve with $d=0$, ie.~pure Gasssian oscillons with no breathing modes.\qquad [DOWN] Numerical map $E_{\rm fini}/E_{\rm init}$ generated from the same algorithm in Chapter \ref{general}. We see that including breathing modes with $d\in(-1,1)$ significantly enlarges the region of stable oscillons, and enables the theoretical map to match the numerical map.}\label{stabilityfull}
\end{figure}

\begin{figure}\centering
  \includegraphics[width=0.7\textwidth]{plot/slow-mode-phi4-1d.png}
  \caption{Heat map of $S \cdot E_{\rm fini}/E_{\rm init}$ for the 1D $\phi^4$ oscillons. (See Chapter \ref{general}). We see that near the boundary of the stable regions, the oscillons have more pronounced slow modes.}\label{phi4slowmode}
\end{figure}
