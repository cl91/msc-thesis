...
\begin{figure}\centering
  ...
  \caption{The full stability map of the 1D $\phi^4$ oscillons. [TOP] Theoretical map with $d\in(-1,1)$. The dashed curve is the stability curve with $d=0$, ie.~pure Gaussian oscillons with no breathing modes.\quad [DOWN] Numerical map $E_{\rm fini}/E_{\rm init}$ generated from the same algorithm in Chapter \ref{general}.\qquad We see that including breathing modes with $d\in(-1,1)$ significantly enlarges the region of stable oscillons, and enables the theoretical map to match the numerical map.}\label{stabilityfull}
\end{figure}

We see from Fig.~\ref{stability1} that the option of having a breathing mode increases the area of stable oscillons. The theoretical region of the $A$-$R$ diagram where stable oscillons exist is the region spanned by the inside of the stability curves with $d\in(-1,1)$. We plot both the theoretical stability map and the numerical $E_{\rm fini}/E_{\rm init})$ map in Fig.~\ref{stabilityfull}, and see that the enlarged theoretical stability map agrees with the stable region in the numerical map --- the original, non-breathing region would be too small to correctly account for the stable region in the numerical map.
